# Author: Ben Wichser
# Date: 12/11/2019
# Description:  Intcode computer BOOST (1).  see www.adventofcode.com/2019 (day 9) for more information


# Plan:
# 10 ) Make grid of spaceship hull (100x100, starting at 50,50 ???) as a dictionary with keys being tuples and values being . or #.  Create direction signifier
# 20) run intcode computer, passing value from dictionary
# 30) capture output twice (color, turn direction)
# 35) turn output into color in dictionary and iterate new location
# 40) repeat 20/30 until we have 1000 paintings.
...
def robot_do_it(grid, painted_locations, current, direction, instruction_list):
    ...
    grid, painted_locations = robot_paint(grid, painted_locations, current, instruction_list)
...
while keep_going:
    while len(instruction_list) < 2:
        ...
        if opcode == 1:
            ...
            intcode_three(parameter_list, code_list, relative_base, grid, current)

        elif opcode == 4:
            instruction_list.append(intcode_four(parameter_list, code_list, relative_base))
    ...
    grid, painted_locations, current, direction  = robot_do_it(grid, painted_locations, current, direction, instruction_list)
    instruction_list = []
    print(painted_locations)
